...
public:
	...
	virtual double evaluate(const VectorXd &u) const {
        ...
        if (QUESTION_12) {
			...
			double left_theta = u[4];
			double right_theta = u[5];

			auto x_prime = get_x_prime();

			Vector2d left_cpos = seg2(x_prime, I[0]);
			Vector2d right_cpos = seg2(x_prime, I[I.size()-1]);

			double ang_l = atan2((left_cpos[1] - right_cpos[1]),(left_cpos[0] - right_cpos[0]));
			double ang_r = atan2((right_cpos[1] - left_cpos[1]),(right_cpos[0] - left_cpos[0]));
			
			double energy_val = pow(ang_r - left_theta, 2) + pow(ang_r - right_theta, 2);
	...
	VectorXd get_DODu(const VectorXd &u) const {
		...
		VectorXd DODu; DODu.setZero(u.size()); {
			...
			if (step == FD_O_of_u) {
				...
				SparseMatrixd minus_dFdx = get_H(u, x);

				MatrixXd dxdu = solve_AX_EQUALS_B(minus_dFdx, dFdu);
		...
		SparseMatrixd ret; ret.resize(x.size(), u.size()); {
			for (const auto &pin : sim->pins) { 
				...
				MatrixXd dp_primedu; dp_primedu.setZero(2, u.size()); {
					...
					bool BELONGS_TO_LEFT_HANDLE = pin.handleIndex == 0; //set to 0, taking left handle
					int xy_dataStartIndex    = (BELONGS_TO_LEFT_HANDLE) ? 0 : 2; //take index of x and y handle coord
					int theta_dataIndex      = (BELONGS_TO_LEFT_HANDLE) ? 4 : 5; //take index of handle theta
					auto theta = u[theta_dataIndex]; //take theta value
					auto o = sim->handleCenters[pin.handleIndex]; //take handle center position of that index (0=left)
					auto s_X = pin.nodePos(0, sim->X); //take the undeformed position of first node
					auto f = from_(o, s_X); //vector from handle center to first node
					// --
					// From updatePinAnchorsAccordingToHandles(...): pin.anchorPosition = o + rotate_(from_(o, s_X), theta) + T;
					// --> p' = o + R_theta{f} + xy;  = handle center + rotated vector from center to node + translation to new center pos
