...
public: 
	...
	double log_c_target = 2.2; double log_c_target_min = -3.; double log_c_target_max = 3.;
	double log_c_regx = 1.001; double log_c_regx_min = 0.85; double log_c_regx_max = 1.15;
	double log_c_regy = 0.96; double log_c_regy_min = 0.85; double log_c_regy_max = 1.15;
	...
	virtual double evaluate(const VectorXd &u_stack) const { 
		...
		Vector2d x_prime = x_sun + Vector2d(-r_prime*pow(2, 0.5)/2, r_prime*pow(2, 0.5)/2);
		//Vector2d x_prime = x_sun + Vector2d(r_prime, 0);
		Vector2d v_prime = Vector2d(-x_prime[1], x_prime[0]).normalized()/pow(r_prime, 0.5);
		v_prime = Vector2d(log_c_regx*v_prime[0], log_c_regy*v_prime[1]);

		
		double O = 0.;
		O += pow(10., log_c_target)*((x.back() - x_prime).squaredNorm() + (v.back() - v_prime).squaredNorm());
...
private: 
    ...
    void drawImGui() override { using namespace ImGui; 
		...
		SliderScalar("log_c_reg_x", ImGuiDataType_Double, &objective.log_c_regx, &objective.log_c_regx_min, &objective.log_c_regx_max); 
		SliderScalar("log_c_reg_y", ImGuiDataType_Double, &objective.log_c_regy, &objective.log_c_regy_min, &objective.log_c_regy_max); 
		SliderScalar("log_c_target", ImGuiDataType_Double, &objective.log_c_target, &objective.log_c_target_min, &objective.log_c_target_max);
		Separator();
