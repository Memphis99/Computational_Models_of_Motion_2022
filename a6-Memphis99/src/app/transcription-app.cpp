...
pair<Vector2d, Vector2d> stepPhysicsExplicitEuler(const Vector2d &xk, const Vector2d &vk, const Vector2d &uk) {
	...
	Vector2d xkp1 = xk + h*vk;
	Vector2d vkp1 = vk + h*Fk/m;
	// TODO: xkp1 = ...
	// TODO: vkp1 = ...
...
public:
	...
	double log_c_reg = -1.6; double log_c_reg_min = -6.; double log_c_reg_max = 6.;
	double log_c_x0 = 4.4; double log_c_x0_min = -6.; double log_c_x0_max = 6.;
	double log_c_physics = 2.6; double log_c_physics_min = -6.; double log_c_physics_max = 6.;
	double log_c_target = 3.4; double log_c_target_min = -6.; double log_c_target_max = 6.;
	...
	virtual double evaluate(const VectorXd &stack) const {
		...
		O += pow(10., log_c_target)*((x.back() - x_prime).squaredNorm() + v.back().squaredNorm());

		O += pow(10., log_c_x0)*((x[0] - x0).squaredNorm() + (v[0] - v0).squaredNorm());

		for (int i=0; i<u.size()-1; i++){
			Vector2d v_dyn = (x[i+1] - x[i])/h;
			Vector2d F_dyn = m*(v[i+1] - v[i])/h;

			O += pow(10., log_c_physics)*((v_dyn - v[i]).squaredNorm() + (F_dyn - u[i]).squaredNorm());

			O += pow(10., log_c_reg)*(u[i].squaredNorm() + x[i].squaredNorm() + v[i].squaredNorm());
		}

		O += pow(10., log_c_reg)*(u.back().squaredNorm() + x.back().squaredNorm() + v.back().squaredNorm());
