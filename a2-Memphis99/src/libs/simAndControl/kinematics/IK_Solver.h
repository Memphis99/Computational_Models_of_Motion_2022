...
public:
    ...
    double ground_y(P3D target) {
        P3D sp = P3D(0, -4.43, 4.5);
        P3D ep = P3D(sp + V3D(0, 0, 3.5));
        double radius = 5;
        ...
        y_sphere1 = sp.y + sqrt(MAX(pow(radius, 2) - pow((sp.x - target.x), 2) - pow((sp.z - target.z), 2), 0));

        y_sphere2 = ep.y + sqrt(MAX(pow(radius, 2) - pow((ep.x - target.x), 2) - pow((ep.z - target.z), 2), 0));

        /*
        if (target.z <= ep.z && target.z >= sp.z){
            y_cylinder = sp.y + sqrt(MAX(pow(radius, 2) - pow((ep.x - target.x), 2), 0));
        }
        */

        return MAX(y_sphere1, MAX(y_sphere2, MAX(y_cylinder, 0)));
    }
    ...
    void addEndEffectorTarget(RB *rb, P3D p, P3D target) {
        ...
        endEffectorTargets.back().target = target + P3D(0, ground_y(target), 0) -  P3D(0, ground_y(robot->root->state.pos), 0);
    }
    ...
    void solve(int nSteps = 10) {
        ...
        for (uint i = 0; i < nSteps; i++) {
            ...
            for (IK_EndEffectorTargets target : endEffectorTargets){

                gcrr.estimate_linear_jacobian(target.p, target.rb, jac);

                jac = jac.block(0,6,3,q.size() - 6).eval();

                P3D pos = gcrr.getWorldCoordinates(target.p, target.rb);

                V3D diff = V3D(pos, target.target);

                deltaq += (jac.transpose() * jac).ldlt().solve(jac.transpose()) * diff;
        ...
        dVector q;
        gcrr.getQ(q);
        q(1) += ground_y(robot->root->state.pos);
        gcrr.setQ(q);
