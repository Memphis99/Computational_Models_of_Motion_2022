...
private:
    ...
    void generate(const bFrameState& startingbFrameState) {
        ...
        while (t < tEnd) {
            ...
            V3D forw_dir = robot->forward;
            V3D sideway_dir = RBGlobals::worldUp.cross(robot->forward);

            V3D vForward_vec = forw_dir * vForward;
            V3D vSideways_vec = sideway_dir * vSideways;
            V3D velocity_vec = vForward_vec + vSideways_vec;

            pos = pos + heading * velocity_vec * dt; /* + TODO: fix this */
            headingAngle +=  turningSpeed * dt; /* + TODO: fix this */
