...
public:
    ...
    double evaluate(const VectorXd& x) const override {
        ...
        Vector2d ee_pos = endEffectorPosition(*linkage, x);

        Vector2d diff = ee_pos - *target;
        
        double e = (pow(diff[0], 2) + pow(diff[1], 2))/2;
    ...
    VectorXd gradient(const VectorXd& x) const override {
        ...
        VectorXd grad = jac.transpose() * diff;
    ...
    Matrix2d hessian(const VectorXd &x) const {
        ...
        Tensor2x2x2 dJ = ddendEffector_ddangles(*linkage, x);

        Vector2d v1 = dJ[0].transpose() * diff;
        Vector2d v2 = dJ[1].transpose() * diff;

        Matrix2d d_jac;

        d_jac << v1, v2;

        Matrix2d hess = jac.transpose() * jac + d_jac;
